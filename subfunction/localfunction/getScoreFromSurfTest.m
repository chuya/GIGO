function [scoreVector] = getScoreFromSurfTest(img,keyPointString)
keyNumber = str2double(keyPointString);
addSurfPaht();
fprintf('Get Local feature Result Program Start...\n');
functionname='getScoreFromSurfTest.m';
...
for topicIndex = 1:length(topicList)
    ...
    descriptorKeyPointPath = [functiondir 'subfunction\temp\tempLocal\prefeature\' char(topicList(topicIndex)) 'key' keyPointString 'Descriptor'];
...
%     end
    ...
    tempScoreList = double(tempScoreList) / pointsNumber ;
    finalScoreList = cat(2,finalScoreList,tempScoreList);
end
